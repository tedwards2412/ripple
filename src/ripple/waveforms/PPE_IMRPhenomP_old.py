...
# same as IMRPhenomP.py but uses PPE_IMRPhenomD instead of IMRPhenomD
...
def time_corr(m1, m2, chi1_l, chi2_l, chip, phiRef, M, dist_mpc, ppes):
    '''
    here m1 > m2
    '''
    ...
    phi_IIb = lambda f: PhenomPOneFrequency_phase(
        f, m1, m2, chi1_l, chi2_l, chip, phiRef, M, dist_mpc, ppes
    )
    t0 = jax.grad(phi_IIb)(f_RD) / (2 * jnp.pi)
    return t0

def time_corr_coarse(m1, m2, chi1_l, chi2_l, chip, phiRef, M, dist_mpc, ppes):
    '''
    here m1 > m2
    '''
    theta_intrinsic = jnp.array([m1, m2, chi1_l, chi2_l])
...
#
    ...
    #)
    #t0 = jax.grad(phi_IIb)(f_RD) / (2 * jnp.pi)
    t0 = time_corr_coarse(m2, m1, chi2_l, chi1_l, chip, phiRef, M, dist_mpc, ppes)
